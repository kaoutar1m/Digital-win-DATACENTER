...
class SensorService {
    ...
    static async createSensor(sensorData) {
        try {
            ...
            const rackCheck = await connection_1.pool.query('SELECT id FROM racks WHERE id = $1', [sensorData.rack_id]);
    ...
    static async updateSensorValue(id, value) {
        ...
        RETURNING *
      `;
            const result = await connection_1.pool.query(query, [value, alert, id]);
            if (result.rows.length === 0) {
                throw new Error(`Sensor with ID ${id} not found`);
            }
            const sensor = result.rows[0];
    ...
    static async updateSensorThreshold(id, threshold) {
        ...
        RETURNING *
      `;
            const result = await connection_1.pool.query(query, [threshold, alert, id]);
            if (result.rows.length === 0) {
                throw new Error(`Sensor with ID ${id} not found`);
            }
            const sensor = result.rows[0];
    ...
    static async deleteSensor(id) {
        try {
            const result = await connection_1.pool.query('DELETE FROM sensors WHERE id = $1 RETURNING id', [id]);
    ...
    static async getSensorsByRack(rackId) {
        ...
        ORDER BY type
      `;
            const result = await connection_1.pool.query(query, [rackId]);
            return result.rows.map((row) => ({
                ...row,
                last_updated: new Date(row.last_updated)
            }));
    ...
    static async getSensorStatistics() {
        ...
        FROM sensors
      `);
            const typeResult = await connection_1.pool.query(`
        SELECT type, COUNT(*) as count
        FROM sensors
        GROUP BY type
      `);
            const avgResult = await connection_1.pool.query(`
    ...
    static async getSensorHistory(id, limit = 50) {
      ...
      `;
            try {
                const result = await connection_1.pool.query(query, [id, limit]);
                if (result.rows.length === 0) {
