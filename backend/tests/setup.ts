...
export const testDbConfig = {
  ...
  database: process.env.TEST_DB_NAME || 'datacenter_test',
  user: process.env.TEST_DB_USER || 'datacenter_user',
  password: process.env.TEST_DB_PASSWORD || 'datacenter_password',
};

export const setupTestDatabase = async (): Promise<Client> => {
  const client = new Client(testDbConfig);
  await client.connect();

  // Clean up existing data
  await client.query('DROP TABLE IF EXISTS alert_comments CASCADE');
  await client.query('DROP TABLE IF EXISTS alert_attachments CASCADE');
  await client.query('DROP TABLE IF EXISTS alert_history CASCADE');
  await client.query('DROP TABLE IF EXISTS alert_notifications CASCADE');
  await client.query('DROP TABLE IF EXISTS alert_rules CASCADE');
  await client.query('DROP TABLE IF EXISTS alerts CASCADE');
  await client.query('DROP TABLE IF EXISTS equipment_history CASCADE');
  await client.query('DROP TABLE IF EXISTS equipment_metrics CASCADE');
  await client.query('DROP TABLE IF EXISTS equipment CASCADE');
  await client.query('DROP TABLE IF EXISTS equipment_models CASCADE');
  await client.query('DROP TABLE IF EXISTS sensors CASCADE');
  await client.query('DROP TABLE IF EXISTS racks CASCADE');
